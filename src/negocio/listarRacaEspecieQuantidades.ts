...
export default class ListarPorEspecieRaca {
    ...
    public listar(): void{
        console.log('Serviços e produtos mais consumidos por espécie e raça de pet: \n');
        ...
        for(const chave in especieRacaData){
            ...
            console.log(`Produto mais consumido: ${data.produto ? data.produto.nome : 'Nenhum produto foi consumido'}`);
            console.log(`Quantidade: ${data.produtoQtd}`);
            console.log(`Serviço mais consumido: ${data.servico ? data.servico.nome : 'Nenhum serviço foi consumido'}`);
